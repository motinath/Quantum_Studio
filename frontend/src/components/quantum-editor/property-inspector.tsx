...
export function PropertyInspector() {
  ...
  if (!sel) {
    return <EmptyState text="Select a placement or connection to inspect its properties." />;
  }
  ...
  return (
    <div className="flex flex-col gap-3 text-xs">
      <div className="flex items-center justify-between border-b border-border pb-2">
        <div>
          <p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Placement</p>
      ...
      <Section title="Position">
        <Field label="Name">
          <Input value={placement.name} onChange={(e) => update({ name: e.target.value })} className="h-7 text-[11px]" />
        </Field>
        <div className="grid grid-cols-2 gap-2">
          <Field label="x (mm)">
            <Input type="number" step="0.05" value={placement.x} onChange={(e) => update({ x: parseFloat(e.target.value) || 0 })} className="h-7 text-[11px]" />
          </Field>
          <Field label="y (mm)">
            <Input type="number" step="0.05" value={placement.y} onChange={(e) => update({ y: parseFloat(e.target.value) || 0 })} className="h-7 text-[11px]" />
          </Field>
        </div>
        <Field label="Rotation (deg)">
          <Select value={String(placement.rotation)} onValueChange={(v) => update({ rotation: parseInt(v, 10) })}>
          ...
          return (
            <Field key={f.name} label={`${f.label}${f.unit ? ` (${f.unit})` : ""}`}>
              {f.kind === "enum" && f.options ? (
                ...
                <Input value={current} onChange={(e) => updateParam(f.name, e.target.value)} className="h-7 font-mono text-[11px]" />
              )}
      ...
      <Section title="Route component">
        {routeOptions.length === 0 ? (
          <Input
            ...
            onChange={(e) =>
              dispatch({ type: "UPDATE_CONNECTION", id: connection.id, patch: { routeComponentId: e.target.value || undefined } })
            }
          ...
          <Select
            value={connection.routeComponentId ?? ""}
            onValueChange={(v) =>
              dispatch({ type: "UPDATE_CONNECTION", id: connection.id, patch: { routeComponentId: v } })
            }
          >
            <SelectTrigger className="h-7 text-[11px]"><SelectValue placeholder="Choose route component" /></SelectTrigger>
            <SelectContent>
              {routeOptions.map((r) => <SelectItem key={r} value={r}>{r}</SelectItem>)}
            </SelectContent>
          </Select>
        )}
