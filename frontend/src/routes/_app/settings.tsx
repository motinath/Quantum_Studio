      ...
      >
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Workspace, backend, AI, and notification preferences.
          </p>
        </div>
        ...
        <Card className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <Label htmlFor="ws-name" className="text-xs font-bold text-slate-600">
                Organization Name
              </Label>
              <Input
                id="ws-name"
              ...
              <div className="mt-1.5 h-9 rounded-xl border border-slate-200 bg-slate-50 flex items-center px-3 gap-2">
                {activeProject ? (
                  <>
                    ...
                    <span className="text-sm text-slate-700 truncate">{activeProject.name}</span>
                  </>
                ) : (
            ...
            <div>
              ...
              <Input
                value={maxQubits}
                onChange={(e) => setMaxQubits(e.target.value)}
                type="number"
                className="mt-1.5 h-9 rounded-xl text-sm border-slate-200"
              />
            </div>
          ...
          <Button
            ...
            className="mt-4 h-9 rounded-xl bg-accent text-white text-xs font-bold px-5"
          >
            {saved === "workspace" ? (
              <>
        ...
        <Card className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            ...
            <Badge
              variant="outline"
              className={`rounded-full text-[9px] font-bold px-2 py-0.5 ${backendOnline ? "bg-emerald-50 text-emerald-700 border-emerald-200" : "bg-rose-50 text-rose-700 border-rose-200"}`}
            >
        ...
        <Card className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div>
            ...
            />
            <p className="text-[10px] text-slate-400 mt-1">
              Set in <code className="font-mono">backend/.env</code> as{" "}
              <code className="font-mono">ANTHROPIC_API_KEY</code>. Without it, the assistant uses
            ...
            {[
              {
                id: "design_events",
                label: "Design events",
                desc: "When a chip is generated or edited",
              },
              {
                id: "verification_alerts",
                label: "Verification alerts",
                desc: "DRC failures and frequency collisions",
              },
              {
                id: "simulation_done",
                label: "Simulation completed",
                desc: "When a simulation job finishes",
              },
              {
                id: "weekly_digest",
                label: "Weekly digest",
        ...
        <Card className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between py-2 border-b border-slate-100">
              ...
              <Badge
                variant="outline"
                className="rounded-full text-[9px] font-bold bg-slate-50 border-slate-200"
              >
                10080 min
              </Badge>
            </div>
            <div className="flex items-center justify-between py-2">
              <div>
                ...
                <p className="text-[10px] text-slate-500">
                  Local JWT — no external OAuth configured
                </p>
              </div>
              <Badge
                variant="outline"
                className="rounded-full text-[9px] font-bold bg-blue-50 text-blue-700 border-blue-200"
              >
                JWT / HS256
              </Badge>
            </div>
