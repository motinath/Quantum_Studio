              ...
              <div>
                ...
                <p className="text-sm text-slate-500">
                  ...
                  {activeProject && (
                    <>
                      {" "}
                      · <strong className="text-accent">{activeProject.name}</strong>
                    </>
                  )}
                </p>
              </div>
            ...
            {!hasDesign && (
              <Card className="rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center">
                ...
                <p className="text-xs text-slate-400 mt-1">
                  Generate a chip in the{" "}
                  <Link to="/designer" className="text-accent hover:underline">
                    Designer
                  </Link>{" "}
                  first.
                </p>
                ...
                <Card className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    ...
                    <div className="flex items-center gap-3">
                      <div className="text-right">
                        ...
                        <p
                          className={cn(
                            "text-2xl font-black",
                            report.summary.yield_estimate >= 90
                              ? "text-emerald-600"
                              : report.summary.yield_estimate >= 70
                                ? "text-amber-600"
                                : "text-rose-600",
                          )}
                        >
                  ...
                  <div className="mt-4 grid grid-cols-4 gap-3">
                    {[
                      ...
                      {
                        label: "DRC",
                        val: report.drc_passed ? "PASS" : "FAIL",
                        color: report.drc_passed ? "text-emerald-600" : "text-rose-600",
                      },
                    ].map((s) => (
                      <div
                        key={s.label}
                        className="text-center rounded-xl border border-slate-100 bg-slate-50 p-3"
                      >
                ...
                {allIssues.length > 0 ? (
                  ...
                  <Card className="rounded-2xl border border-emerald-200 bg-emerald-50/40 p-5 text-center">
                    ...
                    <p className="text-xs text-emerald-600 mt-1">
                      No violations, collisions, or significant crosstalk.
                    </p>
                  </Card>
                )}
              </>
          ...
          <div className="space-y-3">
            {!activeProject && (
              <Card className="rounded-2xl border border-amber-200 bg-amber-50/40 p-4">
                <p className="text-xs text-amber-800 font-semibold flex items-center gap-2">
                  ...
                  Select an active project on the{" "}
                  <Link to="/projects" className="underline">
                    Projects
                  </Link>{" "}
                  page to see saved verification reports.
                </p>
              </Card>
            )}
            ...
            {!loadingHistory && history.length === 0 && activeProject && (
              <Card className="rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center">
                ...
                <p className="text-xs text-slate-400 mt-1">
                  Run a verification to save a report for {activeProject.name}.
                </p>
              </Card>
            )}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-slate-400">
                          {r.summary?.total_issues ?? 0} issues · yield{" "}
                          {r.summary?.yield_estimate ?? 0}%
