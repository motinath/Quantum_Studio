...
function DesignerPage() {
  ...
  if (!hydrated || !active) {
    return (
      <div className="flex h-[calc(100vh-3rem)] w-full items-center justify-center bg-[#F7F8FA]">
        <div className="flex flex-col items-center gap-3">
          <div className="h-10 w-10 rounded-2xl bg-gradient-to-br from-violet-500 to-violet-700 flex items-center justify-center animate-pulse">
            <Atom className="h-5 w-5 text-white" />
          </div>
          <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">Loading designer…</p>
        </div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 min-w-0 h-full overflow-hidden">
        ...
        {/* ─── Chat Panel — 35% ──────────────────────────────────────── */}
        <div
          className="flex flex-col h-full overflow-hidden bg-white border-r border-slate-100"
          style={{ width: hasOutput ? "35%" : "100%" }}
        >
          {/* Chat Header */}
        ...
        <AnimatePresence>
          {hasOutput && result && (
            <motion.div
              ...
              className="flex flex-col h-full bg-[#F7F8FA] overflow-hidden"
              style={{ width: "65%" }}
            >
              <Tabs
                value={view}
