...
def _build_program(
    ...
    freq_map = (freq_plan_dict or {}).get("qubit_frequencies_GHz", {})
    qubits = [
        QubitNode(
            name=f"Q{i+1}",
            qubit_type=qubit_type,
            attributes=[
                Attribute(
                    "frequency",
                    round(freq_map.get(f"Q{i+1}", target_freq + (-0.1 if i % 2 == 0 else 0.1) + (i * 0.013 % 0.06)), 4),
                ),
            ],
        )
...
def _build_qclang_source(
    ...
    for i in range(num_qubits):
        f_val = round(freq_map.get(f"Q{i+1}", target_freq + (-0.1 if i % 2 == 0 else 0.1) + (i * 0.013 % 0.06)), 4)
        freq_attr = f"frequency={f_val}"
