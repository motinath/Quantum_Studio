...
def _build_program(
    ...
    qubit_type: str,
    target_freq: float,
    freq_plan_dict: dict[str, Any] | None = None,
) -> Program:
    # Use 1-indexed names (Q1, Q2, ...) to match topology_router and frequency_planner
    qubit_freqs = (freq_plan_dict or {}).get("qubit_frequencies_GHz", {})
    qubits = [
        QubitNode(
            name=f"Q{i+1}",
            attributes=[
                ...
                Attribute(
                    "frequency",
                    round(qubit_freqs.get(f"Q{i+1}", target_freq + (-0.1 if i % 2 == 0 else 0.1) + (i * 0.013 % 0.06)), 4),
                ),
            ],
        )
...
def _build_qclang_source(
    ...
    qubit_freqs = (freq_plan_dict or {}).get("qubit_frequencies_GHz", {})
    for i in range(num_qubits):
        qubit_freq = qubit_freqs.get(f"Q{i+1}", target_freq + (-0.1 if i % 2 == 0 else 0.1) + (i * 0.013 % 0.06))
        freq_attr = f"frequency={round(qubit_freq, 4)}"
...
def generate_chip(
    ...
    program = _build_program(n, topology, qubit_type, freq, freq_plan_dict)
    chip = program.primary_chip
    code = _generate_code(chip, placement_dict, met) if chip else ""
    qclang_src = _build_qclang_source(n, topology, qubit_type, freq, sub, met, freq_plan_dict)
